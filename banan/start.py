...
def top_coin():
    ...
    for i in trading_pairs:
        try:
            ...
            if data_token[0][-3:] == sorted(data_token[0][-3:]) and sum(data_token[1][:-3])/len(data_token[1][:-3]) * 7 < data_token[1][-1] and 100-(data_token[0][-3]/data_token[0][-1])*100 > 2.4:
                chat_id = -695765690
            ...
            #     chat_id = -695765690
            #     bot = telebot.TeleBot(telega_token)
            #     message = f"PUMP {i}\n{data_token[0][-3:], data_token[1][-3:]}\nРОСТ ЦЕНЫ НА {100-(data_token[0][-3]/data_token[0][-1])*100}%\nVolume average- {sum(data_token[1][:-3])/len(data_token[1][:-3])}"
            #     bot.send_message(chat_id, message)
        except:
            pass
